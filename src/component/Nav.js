...
const Nav = () => {
    ...
    const toggleCart = () => {
        setIsHiddenCart(!isHiddenCart)

    }
    const [isHiddenFav, setIsHiddenFav] = useState(true)
    ...
    return (
        <>
         ...
         {/* ตะกร้า */}
         <div className={`absolute w-full h-full ${isHiddenCart ? "hidden" : "flex"}`} >
                <div onClick={() => toggleCart()} className='flex w-full h-full bg-transparent justify-end px-36 cursor-pointer'>
            ...
            {/* Favlist */}
            <div className={`absolute w-full h-full ${isHiddenFav ? "hidden" : "flex"}`} >
                <div onClick={() => toggleFav()} className='flex w-full h-screen bg-transparent justify-end px-28 '>
                    <div className='flex w-[250px] h-[500px] bg-amber-200    '>
                        <div className='m-1 px-1 w-[240px] h-[490px] bg-white rounded-lg'>
                            <div className='flex justify-between'>
            ...
            {/* hiddenlist */}
            <div className={`absolute w-full h-full  ${isHiddenList ? "hidden" : "flex"}`} >
                <div onClick={() => toggleList()} className='flex w-full h-screen bg-transparent justify-end px-12'>
                    <div className='flex w-[250px] h-[210px] bg-amber-200'>
                        <div className='m-1 px-1 w-[240px] h-[200px] bg-white rounded-lg'>
                            <div className='flex-col w-screen h-screen'>
                            <div className='flex my-1 mx-52 cursor-pointer'>
                            <Close onClick={() => toggleList()} />
                            </div>
                                <il className='flex my-2 mx-1 bg-amber-200 rounded-lg w-56 h-7 cursor-pointer'>
